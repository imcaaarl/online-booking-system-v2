...
const Information = () => {
  ...
  return (
    <div className="grid rounded-xl p-10 shadow-xl ring-2 ring-[#016A70] bg-[#FDFDFD]">
      {/* Banner Image */}
      <img
      ...
      <div className="flex flex-row items-center text-[#2C2C2C]">
        <svg className="size-8 pl-3 text-[#016A70]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
      ...
      <div className="mt-4 flex w-full justify-center">
        <button onClick={openFeedback} className="rounded-lg bg-[#016A70] px-4 py-2 text-white hover:bg-[#014D50]">
          Add Feedback
        </button>
      </div>
      ...
      {/* Get Feedback */}
      <button onClick={getFeedbacks} className="mt-2 mb-2 flex w-full justify-center gap-3 rounded-lg bg-[#016A70] px-4 py-2 text-white hover:bg-[#014D50]">
        Get Feedback
      </button>
      ...
      <div>
        {feedbacks.map((value: any) => (
          <div key={value.id} className="m-2 grid rounded-lg border-1 p-3 bg-[#F4EAE0] text-[#2C2C2C] shadow-lg">
